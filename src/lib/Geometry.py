...
class Point(tuple):
    ...
    def __new__(cls, xy):
        return tuple.__new__(cls, tuple(xy))
    ...
    def __mul__(self, other): return Point(
        [self.x * other.x, self.y * other.y])
...
class Rectangle(tuple):
    ...
    def get_2_point_rect(w, h, offset=(0, 0), scalar=1, shrink_factor=1):
        ...
        if shrink_factor != 1:
            ...
            shrink_offset = (
                dimensions.min_dimension() * (1 - shrink_factor)) / 2
    ...
    def get_rectangle_points(w, h, scalar=1):
        ...
        if scalar != 1:
            pl = [p.scaled(scalar) for p in pl]
        return pl
    ...
    def __add__(self, other):
        # Given 2 rectangles, return the smallest Rectangle that includes all
        # of both
        all_points = list(self) + list(other)

        nll = Point(
            [min([p.x for p in all_points]), min([p.y for p in all_points])])
        nlr = Point(
            [max([p.x for p in all_points]), min([p.y for p in all_points])])
        nur = Point(
            [max([p.x for p in all_points]), max([p.y for p in all_points])])
        nul = Point(
            [min([p.x for p in all_points]), max([p.y for p in all_points])])
    ...
    def get_bounding_rectangle(point_list):
        ...
        # Given a set of coordinates, return the smallest rectangle that
        # contains all points
        nll = Point(
            [min([p.x for p in all_points]), min([p.y for p in all_points])])
        nlr = Point(
    ...
    # TODO: Hack until I convert the Rectangle class to always use this
    # representation
    ...
    def __repr__(self):
        return 'Rectangle: %s | %s to %s | offset: %s color: %s' % \
            (self.points_str(), self.lower_left,
             self.upper_right, self.offset, self.color)
...
class MultiBlock(set):
    ...
    def __init__(self, point_list, name=None, auto_shift_Q1=True, color=None, can_rotate=False):
        ...
        if auto_shift_Q1:
            self.point_list, self.last_offset = Point.get_Q1_shifted(
                self.point_list)
        for p in self.point_list:
            self.add(p)
        self.bounding_rectangle = Rectangle.get_bounding_rectangle(
            self.point_list)
        ...
        if self.can_rotate:
            for i in range(3):
                ...
                r_shape = MultiBlock(
                    new_points, name=new_name, color=color, can_rotate=False)
...
#         return
    ...
    def __repr__(self):
        if self.name:
            return self.name + self.point_str()
...
if __name__ == '__main__':
    ...
    '''  X
        XXX '''
    TshapeUp = MultiBlock(
        [(0, 0), (1, 0), (2, 0), (1, 1)], name='TshapeUp', color='red', can_rotate=True)
    ''' XX '''
